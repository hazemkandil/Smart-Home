...
/*
 * KeyPad_Program.c  –  Smart_Home1 (ATmega32 – keypad MCU)
 *
 *  Created on: Sep 2022
 *      Author: Hazem Mohamed
 *
 *  Description:
 *      Driver for a 4×3 matrix keypad.
 ...
 *           ASCII character sent over USART to Smart_Home2.
 */
 ...
 * input with internal pull-up (pull-up is achieved by writing HIGH
 * to an input pin on AVR). */
 ...
 * or Keypad_Char[KEYPAD_NOT_PRESSED] if nothing is pressed.
 * A 2 ms delay after each row prevents column-read glitches. */
u8 KeyPad_u8GetPress(void){
	u8 Local_u8Col;
	u8 x = KEYPAD_NOT_PRESSED;  /* default: no key detected */
	...
	return Keypad_Char[x];   /* map flat index to character */
}

/* Scan all columns while the given row is driven LOW.
 * Returns the column index (0-based) of the first LOW column found,
 * or KEYPAD_NOT_PRESSED if all columns remain HIGH.
 * Restores the row to HIGH (inactive) before returning. */
...
/* Drive the specified row pin LOW to select (activate) that row.
 * Called before reading columns to enable current-path through pressed key. */
