 ...
 *
 *  Description:
 *      USART driver for the ATmega32 (Smart_Home1 – keypad MCU).
 *      Configures asynchronous serial communication at 9600 bps,
 *      8-bit data, 1 stop bit, no parity.
 *
 *      Smart_Home1 uses USART to send digit characters typed on
 *      the keypad to Smart_Home2, and to receive control tokens:
 *        '-' → first attempt, prompt for credentials
 *        'W' → 1st wrong attempt warning
 *        'N' → 2nd wrong attempt warning
 *        'T' → system locked (3rd failure)
 *        'D' → credentials accepted, door opening
 */
